...
namespace CGAL
{
  ...
  public:
    ...
    typedef Local_kernel::Aff_transformation_3 Local_transform;
    // typedef Local_kernel::Aff_transformation_3<Local_Point> Local_transform;
    ...
    Basic_viewer_qt(QWidget *parent,
                    ...
                    bool draw_text = true,
                    bool no_2D_mode = false) : CGAL::QGLViewer(parent),
                                               ...
                                               m_draw_edges_as_cylinder(!draw_edges),
                                               m_flatShading(true),
                                               ...
                                               // faces for the cylinder
                                               m_buffer_for_cylinder_faces(&arrays[POS_CYLINDER_FACES],
                                                                           nullptr,
                                                                           &m_bounding_box,
                                                                           nullptr,
                                                                           &arrays[FLAT_NORMAL_CYLINDER_FACES],
                                                                           &arrays[SMOOTH_NORMAL_CYLINDER_FACES])
    ...
    void set_draw_edges_as_cylinder(bool b)
    {
      m_draw_edges_as_cylinder = b;
    }
    ...
    bool is_empty() const
    {
      return (m_buffer_for_mono_points.is_empty() &&
              ...
              // faces for cylinder
              m_buffer_for_cylinder_faces.is_empty());
    }
    ...
    bool has_zero_x() const
    {
      return m_buffer_for_mono_points.has_zero_x() &&
             ...
             m_buffer_for_cylinder_faces.has_zero_x() &&
    ...
    bool has_zero_y() const
    {
      return m_buffer_for_mono_points.has_zero_y() &&
             ...
             m_buffer_for_cylinder_faces.has_zero_y() &&
    ...
    bool has_zero_z() const
    {
      return m_buffer_for_mono_points.has_zero_z() &&
             ...
             m_buffer_for_cylinder_faces.has_zero_z() &&
    ...
    bool is_a_face_started() const
    {
      return m_buffer_for_mono_faces.is_a_face_started() ||
             ...
             // Cylinder faces
             m_buffer_for_cylinder_faces.is_a_face_started();
    }
    ...
    void face_begin_cylinder()
    {
      if (m_buffer_for_cylinder_faces.is_a_face_started())
      {
        std::cerr << "You cannot start a new face before to finish the previous one." << std::endl;
      }
      else
      {
        m_buffer_for_cylinder_faces.face_begin();
      }
    }
    ...
    bool add_point_in_face(const KPoint &kp)
    {
      ...
      // Cylinder face
      else if (m_buffer_for_cylinder_faces.is_a_face_started())
      {
        return m_buffer_for_cylinder_faces.add_point_in_face(kp);
      }
    ...
    bool add_point_in_face(const KPoint &kp, const KVector &p_normal)
    {
      ...
      {
        return m_buffer_for_cylinder_faces.add_point_in_face(kp, p_normal);
      }
    ...
    void face_end()
    {
      ...
      {
        return m_buffer_for_cylinder_faces.face_end();
      }
  ...
  protected:
    ...
    Local_transform rotateU(double angle, Local_vector u)
    {
      // float rangle = Math::DegreeToRadian(angle);
      float nc = (1 - cos(angle));
      float c = cos(angle);
      float s = sin(angle);

      Local_transform rotateu = Local_transform((u[0] * u[0]) * nc + c, (u[0] * u[1]) * nc - (s * u[2]), (u[0] * u[2]) * nc + s * u[1],
                                                (u[0] * u[1]) * nc + s * u[2], (u[1] * u[1]) * nc + c, (u[1] * u[2]) * nc - s * u[0],
                                                (u[0] * u[2]) * nc - s * u[1], (u[1] * u[2]) * nc + s * u[0], (u[2] * u[2]) * nc + c);

      return rotateu;
    }

    template <typename KPoint>
    void add_cylinder(const KPoint &p1, const KPoint &p2, float radius = 0.05, int div = 8)
    {
      // cylinder vertices creation
      float dist = CGAL::squared_distance(p1, p2) / 2.0;

      // Vertices
      std::vector<KPoint> diskpoints;

      // Base Disk center (index 0)
      diskpoints.push_back(p1);
      float step = (2 * M_PI) / static_cast<float>(div);
      for (int i = 0; i < div; i++)
      {
        float alpha = step * i;
        float x = p1.x() + radius * cos(alpha);
        float z = p1.z() + radius * sin(alpha);
        diskpoints.push_back(KPoint(x, p1.y(), z));
      }

      // Upper disk center (index div + 1)
      KPoint p1b = KPoint(p1.x(), p1.y() + dist, p1.z());
      diskpoints.push_back(p1b);
      // float dist = squared_distance(p1, p2);

      for (int i = 1; i <= div; i++)
      {
        float alpha = step * i;
        float x = p1b.x() + radius * cos(alpha);
        float z = p1b.z() + radius * sin(alpha);
        diskpoints.push_back(KPoint(x, p1b.y(), z));
      }

      // Applying rotation to each vertex around the direction vector
      Local_vector u = Local_vector();
      Local_vector dir = Local_vector(p2.x() - p1.x(), p2.y() - p1.y(), p2.z() - p1.z());
      // Local_vector dir = (Local_vector(p2) - Local_vector(p1));
      float unorm = sqrt(pow(u.x(), 2) + pow(u.y(), 2) + pow(u.z(), 2));
      float dirnorm = sqrt(pow(dir.x(), 2) + pow(dir.y(), 2) + pow(dir.z(), 2));
      float anglecos = (CGAL::scalar_product(u, dir)) / (unorm * dirnorm);
      double angle = acos(anglecos);
      Local_vector normV = CGAL::cross_product(u, dir);
      Local_transform trans = rotateU(angle, normV);

      /*for (int s = 0; s < diskpoints.size(); s++)
      {
        // diskpoints[s] = trans.transform(diskpoints[s]);
        diskpoints[s] = diskpoints[s].transform(trans);
      }*/
      ...
      {
        face_begin_cylinder();
        add_point_in_face(diskpoints[k]);
        add_point_in_face(diskpoints[0]);
        add_point_in_face(diskpoints[(k % div) + 1]);
        face_end();
      }

      // Upper base triangles
      for (int k = 1; k <= div; k++)
      {
        face_begin_cylinder();
        add_point_in_face(diskpoints[div + 1 + k]);
        add_point_in_face(diskpoints[div + 1]);
        add_point_in_face(diskpoints[div + 1 + (k % div) + 1]);
        face_end();
      }
      ...
      //    | \   |
      //    |  \  |
      //    d --- c      d = (div + 1) + i     c = (div + 1) + (i % div) + 1

      // Sides triangles
      /*for (int i = 1; i <= div; i++)
      {
        face_begin_cylinder();
        add_point_in_face(diskpoints[i]);
        add_point_in_face(diskpoints[(div + 1) + (i % div) + 1]);
        add_point_in_face(diskpoints[(i % div) + 1]);
        face_end();

        face_begin_cylinder();
        add_point_in_face(diskpoints[i]);
        add_point_in_face(diskpoints[(div + 1) + (i % div) + 1]);
        add_point_in_face(diskpoints[(div + 1) + i]);
        face_end();
      }*/
    ...
    void initialize_buffers()
    {
      ...
      // 5.4 ) VAO_CYLINDER_FACES
      vao[VAO_CYLINDER_FACES].bind();

      // 5.4.1) points of the cylinder faces
      ++bufn;
      CGAL_assertion(bufn < NB_VBO_BUFFERS);
      buffers[bufn].bind();
      buffers[bufn].allocate(arrays[POS_CYLINDER_FACES].data(),
                             static_cast<int>(arrays[POS_CYLINDER_FACES].size() * sizeof(float)));
      rendering_program_face.enableAttributeArray("vertex");
      rendering_program_face.setAttributeBuffer("vertex", GL_FLOAT, 0, 3);

      buffers[bufn].release();

      // 5.4.2) normals of the cylinder faces
      ++bufn;
      CGAL_assertion(bufn < NB_VBO_BUFFERS);
      buffers[bufn].bind();
      if (m_flatShading)
      {
        buffers[bufn].allocate(arrays[FLAT_NORMAL_CYLINDER_FACES].data(),
                               static_cast<int>(arrays[FLAT_NORMAL_CYLINDER_FACES].size() *
                                                sizeof(float)));
      }
      else
      {
        buffers[bufn].allocate(arrays[SMOOTH_NORMAL_CYLINDER_FACES].data(),
                               static_cast<int>(arrays[SMOOTH_NORMAL_CYLINDER_FACES].size() *
                                                sizeof(float)));
      }
      rendering_program_face.enableAttributeArray("normal");
      rendering_program_face.setAttributeBuffer("normal", GL_FLOAT, 0, 3);
      ...
      // 5.4.3) color of the cylinder faces
      rendering_program_face.disableAttributeArray("color");
      vao[VAO_CYLINDER_FACES].release();
      ...
      diag_bb = CGAL::squared_distance(Local_point(m_buffer_for_mono_points.bb()->xmin(), m_buffer_for_mono_points.bb()->ymin(), m_buffer_for_mono_points.bb()->zmin()),
                                       Local_point(m_buffer_for_mono_points.bb()->xmax(), m_buffer_for_mono_points.bb()->ymax(), m_buffer_for_mono_points.bb()->zmax()));
      radius_spheres = diag_bb / 400;
    }
    ...
    virtual void draw()
    {
      ...
      if (m_draw_edges_as_cylinder)
      {
        ...
        auto renderer = [this, &color, &clipPlane, &plane_point](float rendering_mode)
        {
          ...
          rendering_program_face.setUniformValue("clipPlane", clipPlane);
          rendering_program_face.setUniformValue("pointPlane", plane_point);
          glDrawArrays(GL_TRIANGLES, 0, static_cast<GLsizei>(arrays[POS_CYLINDER_FACES].size() / 3));
          vao[VAO_CYLINDER_FACES].release();
        };
      ...
      if (m_draw_text)
      {
        ...
        {
          CGAL::qglviewer::Vec screenPos = camera()->projectedCoordinatesOf(CGAL::qglviewer::Vec(std::get<0>(m_texts[i]).x(),
    ...
    virtual void init()
    {
      ...
      float diag_bb = CGAL::squared_distance(Local_point(m_buffer_for_mono_points.bb()->xmin(), m_buffer_for_mono_points.bb()->ymin(), m_buffer_for_mono_points.bb()->zmin()),
                                             Local_point(m_buffer_for_mono_points.bb()->xmax(), m_buffer_for_mono_points.bb()->ymax(), m_buffer_for_mono_points.bb()->zmax()));
    ...
    virtual void keyPressEvent(QKeyEvent *e)
    {
      ...
      {
        if (m_draw_edges && m_draw_edges_as_cylinder)
        {
          m_draw_edges = !m_draw_edges;
          m_draw_edges_as_cylinder = !m_draw_edges_as_cylinder;
        }
        else if (m_draw_edges && !m_draw_edges_as_cylinder)
        {
          m_draw_edges = true;
          m_draw_edges_as_cylinder = true;
        }
        else
        {
          m_draw_edges = true;
          m_draw_edges_as_cylinder = false;
        }
        displayMessage(QString("Draw edges=%1.   draw cylinder=%2.").arg(m_draw_edges ? "true" : "false", m_draw_edges_as_cylinder ? "true" : "false"));
        update();
      }
      ...
      else if ((e->key() == ::Qt::Key_PageUp) && (modifiers == ::Qt::NoButton))
      {
        ...
        displayMessage(QString("Light color=(%1 %2 %3).").arg(m_ambient_color.x()).arg(m_ambient_color.y()).arg(m_ambient_color.z()));
        update();
      }
  ...
  protected:
    ...
    enum
    {
      ...
      SMOOTH_NORMAL_CYLINDER_FACES,
      FLAT_NORMAL_CYLINDER_FACES,
      END_NORMAL,
    ...
    // faces for cylinder
    Buffer_for_vao<float> m_buffer_for_cylinder_faces;
    ...
    static const unsigned int NB_VBO_BUFFERS = (END_POS - BEGIN_POS) +
                                               (END_COLOR - BEGIN_COLOR) + 4; // +4 for 4 vectors of normals (Mono, colored, sphere or cylinder)
